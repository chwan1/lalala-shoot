...
INITIAL_CAMERA_ALTITUDE = 10;//1.7; // Roughly 6 feet tall 
...
/*	if (event.keyCode == 33) {  // Altitude Up   
		...
		altitudeDown = true;
		event.returnValue = false;   
	} else if (event.keyCode == 37) {  // Strafe Left. 
		strafeLeft = true;
		event.returnValue = false;
	} else if (event.keyCode == 39) {  // Strafe Right.    
		strafeRight = true; 
		event.returnValue = false; 
	} */else if (event.keyCode == 69) {  // Tilt Up.   
		tiltUp = true;
		event.returnValue = false;
	} else if (event.keyCode == 67) {  // Tilt Down.   
		tiltDown = true;
		event.returnValue = false;   
	} else if (event.keyCode == 65 ||       
				event.keyCode == 97) { // Turn Left. 
		turnLeft = true;
		event.returnValue = false;   
		 
	} else if (event.keyCode == 68 || 
				event.keyCode == 100) {  // Turn Right.  
		turnRight = true;  
		event.returnValue = false;   
...
/*    if (event.keyCode == 33) {  // Altitude Up    
		...
		strafeRight = false;  
		event.returnValue = false;  
	} else*/ if (event.keyCode == 69) {  // Up.  
		tiltUp = false;
		event.returnValue = false; 
	} else if (event.keyCode == 67) {  // Down.   
		tiltDown = false;  
		event.returnValue = false; 
	} else if (event.keyCode == 65 ||      
				event.keyCode == 97) {  // Left.    
		turnLeft = false; 
		event.returnValue = false;  
	} else if (event.keyCode == 68 ||    
				event.keyCode == 100) {  // Right.  
		turnRight = false; 
		event.returnValue = false; 
	} else if (event.keyCode == 87 ||   
				event.keyCode == 119) {  // Move Forward. 
...
function FirstPersonCam() {  
	...
	// coordinates.  
	me.localAnchorLla = [lat - 0.0005, log, 0];
	...
	// end of frame.   
	google.earth.addEventListener(ge, "frameend", function() { me.update(); });
}
...
FirstPersonCam.prototype.updateOrientation = function(dt) { 
	...
	if (tiltUp || tiltDown) {  
		var tiltSpeed = 30.0; // radians/sec  
		if (tiltDown) 
			tiltSpeed *= -1.0; 
